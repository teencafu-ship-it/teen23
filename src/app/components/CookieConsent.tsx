...
import { fbqConsentGrant, fbqConsentRevoke } from "../lib/fb-pixel";
import { IconCookie } from "./iconsf";
//import { IconX as X, IconCookie as Cookie, IconSettings as Settings, IconCheck as Check, IconShield as Shield } from "./icons";
...
const Cookie = ({ className = "w-6 h-6" }: { className?: string }) => (
  <svg className={className} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const Check = ({ className = "w-6 h-6" }: { className?: string }) => (
  <svg className={className} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default function CookieConsent() {
  ...
  const applyCookies = (prefs: CookiePreferences) => {
    // Apply cookie preferences and wire tracking accordingly
    console.log('تطبيق ملفات تعريف الارتباط:', prefs);

    // If user allowed analytics or marketing, initialize tracking and grant consent
    if (prefs.analytics || prefs.marketing) {
      ...
      console.log('تم تفعيل ملفات تعريف الارتباط التحليلية/التسويقية');
    } else {
      // Otherwise, revoke pixel consent (if pixel loaded)
      try {
        fbqConsentRevoke();
      } catch (e) {
        // ignore
      ...
      console.log('تم تعطيل ملفات تعريف الارتباط التحليلية/التسويقية');
    }
  };
  ...
  const handleAcceptAll = () => {
    ...
    // ensure tracking enabled for analytics/marketing
    try {
      initTracking();
      fbqConsentGrant();
    } catch (e) {
  ...
  const handleAcceptNecessary = () => {
    ...
    // ensure tracking is disabled for analytics/marketing
    try {
      fbqConsentRevoke();
    } catch (e) {
      // ignore
  ...
  const handleTogglePreference = (type: keyof CookiePreferences) => {
    if (type === 'necessary') return; // لا يمكن تعطيل الضرورية
  ...
  const showNotification = (message: string) => {
    ...
    notification.className = 'fixed bottom-24 right-4 bg-emerald-600 text-white px-4 py-3 rounded-lg shadow-lg animate-fade-in z-50';
  ...
  const cookieTypes = [
    {
      id: 'necessary',
      title: 'ضرورية',
      description: 'ملفات تعريف الارتباط الضرورية لتشغيل الموقع بشكل صحيح. لا يمكن تعطيلها.',
      alwaysEnabled: true
    },
    {
      id: 'analytics',
      title: 'تحليلية',
      description: 'تساعدنا على فهم كيفية تفاعل الزوار مع الموقع لتحسين تجربة المستخدم.',
      enabled: preferences.analytics
    },
    {
      id: 'marketing',
      title: 'تسويقية',
      description: 'تستخدم لتقديم إعلانات مخصصة بناءً على اهتماماتك وتفضيلاتك.',
      enabled: preferences.marketing
    },
    {
      id: 'preferences',
      title: 'تفضيلات',
      description: 'تذكر إعداداتك الشخصية وتفضيلاتك على الموقع.',
            ...
            <div className="bg-gradient-to-r from-emerald-600 to-teal-500 p-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <IconCookie className="w-8 h-8 text-white" />
                  <div>
            ...
            <div className="p-6">
              <div className="flex flex-col md:flex-row gap-6">
                <div className="md:w-2/3">
                  <p className="text-gray-700 mb-4">
                    نستخدم ملفات تعريف الارتباط لتحسين تجربتك على موقعنا. بعض ملفات تعريف الارتباط ضرورية 
                    لعمل الموقع، والبعض الآخر يساعدنا على فهم كيفية تفاعلك مع الموقع.
                  </p>
                      ...
                      {cookieTypes.map((type) => (
                        <div key={type.id} className="flex items-center justify-between">
                          ...
                          <div className={`w-6 h-6 rounded flex items-center justify-center ${
                            (type.id === 'necessary' || preferences[type.id as keyof CookiePreferences]) 
              ...
              <div className="mt-6 pt-6 border-t border-gray-200">
                <p className="text-xs text-gray-500 text-center">
                  ...
                  يمكنك الاطلاع على <a href="/privacy" className="text-emerald-600 hover:underline">سياسة الخصوصية</a> و 
                  <a href="/cookies" className="text-emerald-600 hover:underline"> سياسة ملفات تعريف الارتباط</a>.
                </p>
              </div>
            </div>
          </div>
